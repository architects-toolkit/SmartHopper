    ...
    public abstract class AIProvider : IAIProvider
    {
        ...
        public virtual async Task InitializeProviderAsync()
        {
            try
            {
                ...
                if (!AIModelCapabilityRegistry.Instance.HasProviderCapabilities(this.Name))
                {
            ...
            // Settings are retrieved through the host-injected per-provider settings store so
            // the SDK does not depend on the host's SmartHopperSettings type.
            var settingsStore = ProviderSdkHost.SettingsStoreFactory(this.Name);
            var settingsDict = settingsStore?.GetAll() ?? new Dictionary<string, object>();
            ...
            this.RefreshCachedSettings(settingsDict is Dictionary<string, object> concrete
                ? concrete
                : new Dictionary<string, object>(settingsDict));
        }
        ...
        {
            var ui = ProviderSdkHost.ProviderRegistry.GetProviderSettings(this.Name);
        ...
        public string GetDefaultModel(AICapability requiredCapability = AICapability.Text2Text, bool useSettings = true)
        {
            ...
            if (useSettings && this.GetSetting<string>("Model") != null)
            {
                ...
                {
                    if (AIModelCapabilityRegistry.Instance.ValidateCapabilities(this.Name, modelFromSettings, requiredCapability))
                    {
        ...
        public virtual string SelectModel(AICapability requiredCapability, string requestedModel)
        {
            ...
            var selected = AIModelCapabilityRegistry.Instance.SelectBestModel(this.Name, requestedModel, requiredCapability, preferredDefault);
        ...
        protected void SetSetting(string key, object value)
        {
            ...
            try
            {
                ...
                // Persist to the global settings via the host-injected per-provider store
                var store = ProviderSdkHost.SettingsStoreFactory(this.Name);
                store?.Set(key, value);
        ...
        protected JArray GetFormattedTools(string toolFilter)
        {
            try
            {
                ...
                var tools = ProviderSdkHost.ToolRegistry.GetTools();
                if (tools.Count == 0)
                {
        ...
        protected void ApplyOpenAICompatibleToolChoice(
            JObject requestBody,
        ...
        protected AIMetrics DecodeOpenAICompatibleMetrics(JObject response)
        {
            if (response == null)
            {
        ...
        protected bool TryWrapJsonSchema(
            string jsonSchema,
