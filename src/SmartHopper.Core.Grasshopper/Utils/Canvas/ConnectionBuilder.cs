...
namespace SmartHopper.Core.Grasshopper.Utils.Canvas
{
    ...
    public static class ConnectionBuilder
    {
            ...
            bool redraw = true)
        {
            ...
            if (doc == null)
            {
                Debug.WriteLine("[ConnectionBuilder] No active Grasshopper document found.");
                return false;
            }

            // Find source and target objects
            var sourceObj = CanvasAccess.FindInstance(sourceGuid);
            var targetObj = CanvasAccess.FindInstance(targetGuid);

            if (sourceObj == null || targetObj == null)
            {
                Debug.WriteLine($"[ConnectionBuilder] Component not found. Source: {sourceObj != null}, Target: {targetObj != null}");
                return false;
            }
            ...
            {
                Debug.WriteLine($"[ConnectionBuilder] Parameter not found. Source param: {sourceParam != null}, Target param: {targetParam != null}");
                return false;
            }
            ...
            {
                Debug.WriteLine($"[ConnectionBuilder] Connection already exists: {sourceParam.NickName} → {targetParam.NickName}");
            ...
            try
            {
                ...
                Debug.WriteLine($"[ConnectionBuilder] Connected {sourceParam.NickName} ({sourceGuid}) → {targetParam.NickName} ({targetGuid})");
            ...
            {
                Debug.WriteLine($"[ConnectionBuilder] Connection failed: {ex.Message}");
                return false;
            }
        }
