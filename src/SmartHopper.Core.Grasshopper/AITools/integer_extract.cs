...
{
    /// <summary>
    /// Provides a tool for extracting integer values from text.
    /// </summary>
    public class integer_extract : IAIToolProvider
    {
        /// <summary>
        /// Name of the AI tool provided by this class.
        /// </summary>
        private readonly string toolName = "integer_extract";
        ...
        private readonly string systemPrompt =
            "You are an integer extraction assistant. Your task is to extract integer values from text.\n\n" +
            "Respond with only a single integer, nothing else. Round to the nearest integer if necessary. If no integer can be extracted, respond with '0'.";
        ...
        public IEnumerable<AITool> GetTools()
        {
            yield return new AITool(
                name: this.toolName,
                description: "Extracts an integer value from input text",
                category: "DataProcessing",
                parametersSchema: @"{
                    ""type"": ""object"",
                    ""properties"": {
                        ""input"": { ""type"": ""string"", ""description"": ""The input text to extract an integer from"" },
                        ""context"": { ""type"": ""string"", ""description"": ""Optional context for integer extraction (e.g., 'extract the count')"" },
                        ""instructions"": { ""type"": ""string"", ""description"": ""Optional system instructions for the AI"" }
                    },
                    ""required"": [""input""]
                }",
                execute: this.ExtractInteger,
        ...
        private AIRequestCall BuildExtractRequest(AIToolCall toolCall)
        {
            ...
            var userPrompt = string.IsNullOrEmpty(context)
                ? $"Extract an integer from the following text:\n\n{input}"
                : $"Extract an integer from the following text. Context: {context}\n\nText: {input}";
        ...
        /// <summary>
        /// Extracts an integer value from input text.
        /// </summary>
        /// <param name="toolCall">The tool call information.</param>
        /// <returns>AIReturn with the extracted integer.</returns>
        private async Task<AIReturn> ExtractInteger(AIToolCall toolCall)
        {
            ...
            try
            {
                Debug.WriteLine("[IntegerTools] Running ExtractInteger tool");
                ...
                toolResult.Add("integer", response.Trim());
            ...
            {
                Debug.WriteLine($"[IntegerTools] Error in ExtractInteger: {ex.Message}");
