...
{
    /// <summary>
    /// Retrieves a McNeel forum topic by ID and wraps it into an AIInputPayload for AI processing.
    /// Uses the mcneel_forum_topic_get AI tool with preset URL (https://discourse.mcneel.com).
    /// </summary>
    public class McNeelTopic2AIComponent : StatefulComponentBase
    {
        ...
        public McNeelTopic2AIComponent()
            : base(
                "McNeel Topic to AI",
                "McNeelTopic2AI",
        ...
        {
            pManager.AddIntegerParameter("Topic ID", "T", "ID(s) of the forum topic(s) to retrieve from discourse.mcneel.com.", GH_ParamAccess.tree);
        ...
        {
            return new McNeelTopic2AIWorker(this, this.AddRuntimeMessage, this.ComponentProcessingOptions);
        }

        private sealed class McNeelTopic2AIWorker : AsyncWorkerBase
        {
            private readonly McNeelTopic2AIComponent parent;
            ...
            public McNeelTopic2AIWorker(McNeelTopic2AIComponent parent, Action<GH_RuntimeMessageLevel, string> addRuntimeMessage, ProcessingOptions options)
            ...
            public override void GatherInput(IGH_DataAccess DA, out int dataCount)
            {
                ...
                // Convert integer trees to string trees for unified processing
            ...
            public override async Task DoWorkAsync(CancellationToken token)
            {
                ...
                {
                    Debug.WriteLine($"[McNeelTopic2AIWorker] Error: {ex.Message}");
            ...
            private async Task<Dictionary<string, List<IGH_Goo>>> ProcessBranches(Dictionary<string, List<GH_String>> branches)
            {
                ...
                for (int i = 0; i < topicIds.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Endpoint = "mcneel_forum_topic_get",
                        };
