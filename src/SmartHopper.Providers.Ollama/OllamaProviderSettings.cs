            ...
            if (settings.TryGetValue("ServerUrl", out var serverUrlObj) && serverUrlObj != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(serverUrl))
                {
                        ...
                        || (parsed.Scheme != Uri.UriSchemeHttp && parsed.Scheme != Uri.UriSchemeHttps))
                    {
                        if (showErrorDialogs)
                        {
                            ProviderSdkHost.Diagnostics.Report(this.GetType().Name, new SHRuntimeMessage(SHRuntimeMessageSeverity.Error, SHRuntimeMessageOrigin.Validation, SHMessageCode.InputInvalid, "Base URL must be an absolute HTTP or HTTPS URL (for example http://localhost:11434/v1)."));
                        }
