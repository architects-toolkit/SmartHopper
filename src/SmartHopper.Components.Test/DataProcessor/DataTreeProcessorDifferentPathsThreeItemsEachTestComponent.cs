...
namespace SmartHopper.Components.Test.DataProcessor
{
    ...
    public class DataTreeProcessorDifferentPathsThreeItemsEachTestComponent : StatefulComponentBase
    {
        ...
        public DataTreeProcessorDifferentPathsThreeItemsEachTestComponent()
            : base(
                "Test DataTreeProcessor (Different Paths, 3 items each)",
                "TEST-DTP-DIFF-3",
        ...
        private sealed class Worker : AsyncWorkerBase
        {
            ...
            public override async Task DoWorkAsync(CancellationToken token)
            {
                try
                {
                    ...
                    bool ok =
                        ...
                        this._resultTree.get_Branch(pathA) != null && this._resultTree.get_Branch(pathA).Count == 3 &&
                        this._resultTree.get_Branch(pathA)[0] is GH_Integer giA0 && giA0.Value == 1 &&
                        this._resultTree.get_Branch(pathA)[1] is GH_Integer giA1 && giA1.Value == 2 &&
                        this._resultTree.get_Branch(pathA)[2] is GH_Integer giA2 && giA2.Value == 3 &&
                        this._resultTree.get_Branch(pathB) != null && this._resultTree.get_Branch(pathB).Count == 3 &&
                        this._resultTree.get_Branch(pathB)[0] is GH_Integer giB0 && giB0.Value == 4 &&
                        this._resultTree.get_Branch(pathB)[1] is GH_Integer giB1 && giB1.Value == 5 &&
                        this._resultTree.get_Branch(pathB)[2] is GH_Integer giB2 && giB2.Value == 6;

                    this._success = new GH_Boolean(ok);
                    this._messages.Add(new GH_String($"Different paths A={pathA}, B={pathB}. A=[1,2,3], B=[4,5,6]. Expected passthrough: branch {pathA} -> [1,2,3], branch {pathB} -> [4,5,6]. No cross-path combination."));
            ...
            public override void SetOutput(IGH_DataAccess DA, out string message)
            {
                ...
                message = this._success.Value ? "Processed different-path trees (3 each) successfully" : "Processing failed";
            }
        }
    }
}
