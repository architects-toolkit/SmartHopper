...
namespace SmartHopper.Components.Test.Providers
{
    ...
    public class TestDeepSeekDecodeComponent : AIStatefulAsyncComponentBase
    {
        ...
        private sealed class Worker : AsyncWorkerBase
        {
            ...
            private async Task<bool> TestTextDecoding(DeepSeekProvider provider)
            {
                try
                {
                    ...
                    {
                        this._messages.Add(new GH_String("✗ Reasoning content not decoded"));
                        return false;
                    }

                    if (reasoningText.Reasoning == null || !reasoningText.Reasoning.Contains("Let me think"))
                    {
                        this._messages.Add(new GH_String("✗ Reasoning not extracted correctly"));
                        return false;
                    }

                    this._messages.Add(new GH_String("✓ Text decoding successful"));
                    this._messages.Add(new GH_String("✓ Simple text response decoded"));
                    this._messages.Add(new GH_String("✓ Reasoning content extracted"));
                    return true;
                }
                catch (Exception ex)
                {
            ...
            private async Task<bool> TestToolCallDecoding(DeepSeekProvider provider)
            {
                try
                {
                    ...
                    {
                        this._messages.Add(new GH_String($"✗ Tool call ID incorrect: expected 'call_xyz789', got '{toolCall.Id}'"));
                        return false;
                    }

                    if (toolCall.Arguments == null || toolCall.Arguments["query"]?.ToString() != "test")
                    {
            ...
            private async Task<bool> TestErrorDecoding(DeepSeekProvider provider)
            {
                try
                {
                    ...
                    // Test error format 3: Invalid request error with code
                    ...
                    this._messages.Add(new GH_String("✓ Invalid request error with code handled"));
                    return true;
                }
                catch (Exception ex)
                {
            ...
            private async Task<bool> TestMetricsExtraction(DeepSeekProvider provider)
            {
                try
                {
                    // DeepSeek has extended metrics with cached tokens and reasoning tokens
                    ...
                    // DeepSeek calculates InputTokensPrompt = total - cached
                    if (metrics.InputTokensPrompt != 40) // 50 - 10
                    {
                        this._messages.Add(new GH_String($"✗ Input tokens incorrect: expected 40, got {metrics.InputTokensPrompt}"));
                        return false;
                    }

                    if (metrics.InputTokensCached != 10)
                    {
                        this._messages.Add(new GH_String($"✗ Cached tokens incorrect: expected 10, got {metrics.InputTokensCached}"));
                        return false;
                    }
                    ...
                    if (metrics.OutputTokensReasoning != 5)
                    {
                        this._messages.Add(new GH_String($"✗ Reasoning tokens incorrect: expected 5, got {metrics.OutputTokensReasoning}"));
                        return false;
                    }
