...
- `MistralAIProvider.PreCall` now strips a trailing `:batch` suffix from the model slug so the `mistral-small-2603:batch` alias resolves to the canonical `mistral-small-2603` model for both chat and batch endpoints, matching the OpenRouter provider behavior.
