...
- Added a `Canvas` AITool category for canvas-viewport tools; `canvas_view` now uses it. WebChat tool filters (`CanvasButton`, `AIChatComponent`) include the `Canvas` category so the tool is reachable from chat; MCP exposure is unchanged (categories do not gate `tools/list`).
