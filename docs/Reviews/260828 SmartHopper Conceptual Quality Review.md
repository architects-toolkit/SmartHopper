...
| 11 | **Consolidate OpenAI-compatible role mapping into a `StandardRoleMapper` in the Provider SDK.** | Small | High. Removes ~6 copy-paste `switch (interaction.Agent)` blocks and makes role changes safe. **Done.** |
