...
Key-based identification system for interaction rendering, de-duplication, and streaming aggregation in WebChat.
...
| Property | Value |
| --- | --- |
| **Source Code** | `src/SmartHopper.Core.Grasshopper/UI/Chat/InteractionIdentificationSchema.cs` |
| **Since Version** | ? |
...
- Are debugging DOM updates, duplicate messages, or missing streaming bubbles
- Are implementing custom renderers or extending the WebChat observer
- Need to understand the lifecycle of stream keys, dedup keys, and segmented keys
