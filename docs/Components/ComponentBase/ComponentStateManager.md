...
- **Hash tracking.** Holds two dictionaries â€” _committed_ (last successfully processed) and _pending_ (current solve). `GetChangedInputs()` returns the per-input diff (or empty during restoration / suppression).
